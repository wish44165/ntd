...
class NTD:
    ...
    def online_train(self, cur_iter, batch_size, n_worker, seed):
        ...
        for batch_idx, data in enumerate(dataloader):
            ...
            x = data["image"]
            y = data["label"]
            self.tta_cropSize = int(x.shape[-1]*3/4)
            if cur_iter > 0:
                # Create Memory Set.
                memset = ImageDataset(
                    pd.DataFrame(self.memory_list),
            ...
            print('len(self.streamed_list) =', len(self.streamed_list))
            print('batch_idx * batch_size =', batch_idx * batch_size)
            print('(batch_idx + 1) * batch_size =', (batch_idx + 1) * batch_size)
        ...
        logger.info(f"\n{memory_df.klass.value_counts(sort=True)}")    # number of true_label for each klass in memory list

    def online_update_memory(self, cur_data_list):
        print('len(cur_data_list) =', len(cur_data_list))
        ...
        elif self.mem_manage == "NTD":
            self.NTD_sampling(cur_data_list)
    ...
    def cal_loss(self, df, get_loss=True, get_feature=True, test_batchsize=256):
        dataset = ImageDataset(
            df, dataset_path=self.dataset_path, transform=self.test_transform
        )
        dataloader = DataLoader(dataset, batch_size=min(test_batchsize, len(dataset)), shuffle=False)
        ...
        criterion = nn.CrossEntropyLoss(reduction='none')
        #if self.mem_manage == "NTD":    ######## SCELoss - 1 ########
            #criterion = SCELoss(alpha=1.0, beta=1.0, num_classes=self.n_classes)
        criterion = criterion.to(self.device)
        self.model.eval()

        # tta
        tta_model = tta.ClassificationTTAWrapper(self.model, tta.aliases.five_crop_transform(self.tta_cropSize, self.tta_cropSize))    ######## TTA - 1 ########
        ...
        with torch.no_grad():
            ...
            for batch_idx, data in enumerate(dataloader):
                ...
                # tta    ######## TTA - 2 ########
                _, feature = self.model(x, get_feature=True)    # logit = self.model(x)
                logit = tta_model(x)
            ...
            if get_feature:
                ...
                df["feature"] = features
            """
            if self.mem_manage == "NTD":    ######## logit - 1 ########
    ...
    def get_group_info(self, df):

        # 1. feature vectors (or logits)
        features = np.array(df["feature"].tolist())
        #features = np.array(df["logit"].tolist())    ######## logit - 2 ########

        # 2. Preprocess feature vectors
        """
        ...
        # kmeans
        num_clusters = 10
        kmeans = KMeans(n_clusters=num_clusters, random_state=1510384)
        clusters = kmeans.fit_predict(features)
        centroids = kmeans.cluster_centers_
        # Evaluate clustering performance
        silhouette_score_value = silhouette_score(features, clusters)
        # Print cluster labels and silhouette score
        print("Silhouette Score:", silhouette_score_value)
        """

        # 3.2. similarity in each group
        df['similarity'] = np.zeros(len(features))
        for ci in range(self.n_classes):
            group_features = np.array(df[df['label']==ci]['feature'].tolist())
            #group_features = np.array(df[df['label']==ci]['logit'].tolist())    ######## logit - 3 ########
            group_sim_matrix = cosine_similarity(group_features)
            group_sim_score = group_sim_matrix.mean(axis=1)
            df.loc[df['label']==ci, 'similarity'] = group_sim_score

        self.construct_table = False

        return df

    def get_part_group_info(self, df, cls_new, cls_drop):
        CList = [cls_new, cls_drop]
        #print(CList)
        if cls_new == cls_drop:
            group_features = np.array(df[df['label']==cls_new]['feature'].tolist())
            group_sim_matrix = cosine_similarity(group_features)
            group_sim_score = group_sim_matrix.mean(axis=1)
            df.loc[df['label']==cls_new, 'similarity'] = group_sim_score
        else:
            for ci in CList:
                group_features = np.array(df[df['label']==ci]['feature'].tolist())
                group_sim_matrix = cosine_similarity(group_features)
    ...
    # proposed method
    ################################
    def NTD_sampling(self, cur_data_list):    # Noisy label grouping, Test time augmentation, Demographic parity
        for sample in cur_data_list:

            # record seen data for each class
            if sample["label"] in self.seen_per_cls:
                self.seen_per_cls[sample["label"]] += 1
            else:
                self.seen_per_cls[sample["label"]] = 1

            # add sample if memory_list still have vacancy
            if len(self.memory_list) < self.memory_size:
                self.memory_list += [sample]
            
            # sum(cls_cnt) > 500
            else:
                if self.construct_table:
                    # grouping
                    cand_df = pd.DataFrame(self.memory_list + [sample])    # 501 samples
                    cand_df = self.cal_loss(cand_df)    # len(cand_df["feature"][0]) = 512
                    #cand_df = self.get_group_info(cand_df)    ######## feature vectors or logits - 1 ########

                    # demographic parity
                    cls_cnt = cand_df["label"].value_counts()
                    cls_to_drop = cls_cnt.idxmax()    # largest group

                    # drop depends on different characteristic
                    #drop_idx = cand_df[cand_df['label']==cls_to_drop]['similarity'].idxmin()
                    drop_idx = cand_df[cand_df['label']==cls_to_drop]['loss'].idxmax()
                    
                    cand_df = cand_df.drop(drop_idx)    # comment out while verify

                    # verify drop index
                    """
                    print('-'*40)
                    pd.set_option('display.max_rows', self.memory_size+1)
                    print(cand_df)

                    print(drop_idx)
                    keys = np.array(cand_df[cand_df['label']==cls_to_drop]['similarity'].index)
                    values = np.array(cand_df[cand_df['label']==cls_to_drop]['similarity'])
                    checkDict = dict(zip(keys, values))
                    print(checkDict)
                    print(cand_df[cand_df['label']==cls_to_drop]['similarity'].min())

                    cand_df = cand_df.drop(drop_idx)
                    pd.set_option('display.max_rows', self.memory_size)
                    print(cand_df)
                    sys.exit()
                    """
                    ...
                    self.construct_table = False    ######## comment if use feature vectors or logits ########
                else:
                    # grouping
                    sample_df = pd.DataFrame([sample])
                    sample_df = self.cal_loss(sample_df)
                    sample_cls = sample['label']
                    cand_df = pd.concat([self.table, sample_df], axis=0)
                    cand_df = cand_df.reset_index().drop('index', 1)
                    cls_cnt = cand_df['label'].value_counts()
                    cls_to_drop = cls_cnt.idxmax()
                    #cand_df = self.get_part_group_info(cand_df, sample_cls, cls_to_drop)    ######## feature vectors or logits - 2 ########
                ...
                #cand_df = cand_df.drop('logit', 1)    ######## logit - 4 ########
